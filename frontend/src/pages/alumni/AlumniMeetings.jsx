...
export default function AlumniMeetings() {
  ...
  const fetchMeetings = async () => {
    try {
      const res = await api.get('/alumni/meetings')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode && selectedMeeting) {
        await api.put(`/alumni/meetings/${selectedMeeting.id}`, formData)
      } else {
        await api.post('/alumni/meetings', formData)
      }
      setShowModal(false)
      resetForm()
      fetchMeetings()
  ...
  const handleDelete = async (id) => {
    if (confirm('Are you sure you want to delete this meeting?')) {
      try {
        await api.delete(`/alumni/meetings/${id}`)
        fetchMeetings()
      } catch (err) {
  ...
  const isCreator = (meeting) => meeting.createdBy === currentUser?.id
